...
def test_m5_tables_force_rls() -> None:
    ...
    assert len(rows) == len(M5_TABLES)
    assert all(enabled and forced for _, enabled, forced in rows)


def test_m5_runtime_role_has_dml_privileges() -> None:
    with psycopg.connect(OWNER_URL) as conn, conn.cursor() as cur:
...
def test_m5_tables_have_runtime_tenant_policy() -> None:
    with psycopg.connect(OWNER_URL) as conn, conn.cursor() as cur:
        cur.execute(
            """
