...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Owner-only explicit STUDENT role assignment."
    )
    ...
    with psycopg.connect(OWNER_URL) as conn, conn.cursor() as cur:
        ...
        if not rows:
            raise SystemExit(
                "No active student membership found for that login."
            )
        ...
        membership_id, institution_id, student_profile_id = rows[0]
        cur.execute(
            """
            SELECT id
            FROM roles
        ...
        role = cur.fetchone()
        if role is None:
            raise SystemExit("STUDENT role missing; migrate to 0013_m11 first.")
    ...
    print("STUDENT assignment: OK")
    print(f"institution_id={institution_id}")
    print(f"membership_id={membership_id}")
    print(f"student_profile_id={student_profile_id}")
